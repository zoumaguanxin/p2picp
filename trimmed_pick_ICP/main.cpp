...
//用于Trimmed ICP排序,sort函数会调佣函数指针所指向的函数，并输入当前输入进行分析，如果前一个比后以后小，则不会发生顺序的变化。
struct element{
float data;
size_t index;
};
//注意必须是返回int。这是因为qsort函数输入的类型就必须是int
...
int main(int arg, char** argv)
{
	...
	//定义基本的变量
	//*********************************************************************************
        char dir[100]="/home/shaoan/projects/SLAM6D/dat_et4/";//不能写成string dir="H:\\"；在下面sptintf_s输出时会出现乱码，ubuntu下写成/即可，windows下写成“\\”
...
//数据点关联，方案1
	...
	while(convergence_counter<=2&& iterative_counter<counter_max){
		...
		int counter=0;		
		//trimed ICP
		const size_t num_points=nPoint_current;
		pcl::PointXYZ ordered_basic_points[num_points];
		pcl::PointXYZ ordered_local_points[num_points];
		//size_t initialPoints_index[k];
		size_t matchedPoints_index[num_points];
		element order_pointNKNSquareDdistance[num_points];//这是一个结构体，用来存在距离以及它的下标，以便在实现数组排序的同时，对下标的顺序也进行相应的输出。
		for(int i=0;i<nPoint_current;i++)
		{
		  kdtree.nearestKSearch (middle_cloud_ptr->points[i], K, pointIdxNKNSearch, pointNKNSquaredDistance);
		  ordered_basic_points[i]=basic_cloud_ptr2->points[i];
		  ordered_local_points[i]=local_cloud_ptr->points[pointIdxNKNSearch[0]];
		  order_pointNKNSquareDdistance[i].data=pointNKNSquaredDistance[0];
		  order_pointNKNSquareDdistance[i].index=i;
		  matchedPoints_index[i]=pointIdxNKNSearch[0];
		}
		qsort(order_pointNKNSquareDdistance, num_points,sizeof(order_pointNKNSquareDdistance[0]),compare);
		...
		{
		// cout<<"distance:"<<order_pointNKNSquareDdistance[i].data<<"         index:"<<order_pointNKNSquareDdistance[i].index<<endl;
		         if(i==0)
			    {
			       filter_initPoints->points.push_back(ordered_basic_points[order_pointNKNSquareDdistance[i].index]);
		               filter_matchedPoints->points.push_back(ordered_local_points[order_pointNKNSquareDdistance[i].index]);
				indice[0]=matchedPoints_index[order_pointNKNSquareDdistance[i].index];	
				//cout<<"the fist"<<endl;
			    }
			    else
			    {    //cout<<"label 2"<<endl;
				 for (size_t ii=0;ii<=n_filter;ii++)
				  {    // cout<<"die in"<<ii<<endl;
					  if(ii<n_filter){
					    
						    if(indice[ii]==matchedPoints_index[order_pointNKNSquareDdistance[i].index])
						    {     
								      n_filter--;
								   //   cout<<"deleted:"<<i<<endl;
								      break;
					     ...
					     //  cout<<"label 4"<<endl;
					    //cout<<order_pointNKNSquareDdistance[i].index<<endl;
					    //cout<< ordered_basic_points[12881].x<<endl;
					    // size_t med=order_pointNKNSquareDdistance[i].index;
					     //cout<<med<<endl;
					      //cout<< ordered_basic_points[med].x;//<< ordered_basic_points[order_pointNKNSquareDdistance[i].index].y<< ordered_basic_points[order_pointNKNSquareDdistance[i].index].z;
					    filter_initPoints->points.push_back(ordered_basic_points[order_pointNKNSquareDdistance[i].index]);
					      //   cout<<"label 5"<<endl;
					    filter_matchedPoints->points.push_back(ordered_local_points[order_pointNKNSquareDdistance[i].index]);
					     //cout<<"label 3"<<endl;					    
					  indice[ii]=matchedPoints_index[order_pointNKNSquareDdistance[i].index];
					   // cout<<"new data are added"<<endl;
					    break;
					  }
				  }
			    }
			    n_filter++;    
			    // cout<<"going on"<<n_filter<<endl;
		}
		//delete []indice;
	     //  cout<<"it's OK"<<endl;
	...
	//收敛准则
	//*******************************************************************************************
	if(abs(error-error2)/(float)filter_k<0.2&&(error/(float)filter_k<sigma)) convergence_counter=1;
	error2=error;	
