...
//数据点关联，方案1
	...
	int counter_max=100;
	Eigen::Vector3f last_deltaT;
	Eigen::Vector3f current_delatT;
...
L1:            j = t % nPoint_current;
	...
	for(int j=0;j<k;++j)
	{
		...
		error+=sqrt(temV1.dot(temV1));
		  ...
		  //************************************************************RMT方法改变阈值
		  if (iterative_counter==1)
		  {
		    current_delatT=t_update-t_last;
		  }
		  else if(iterative_counter>1){
			  last_deltaT=current_delatT;
			  current_delatT=t_update-t_last;
			  float simulated_anealing;
			  simulated_anealing=current_delatT.squaredNorm()/last_deltaT.squaredNorm()+0.5;
			  cout<< simulated_anealing<<endl;
			  if(simulated_anealing<1)
			  {
			    search_r_up*=simulated_anealing;
			  }
			  cout<<search_r_up<<endl;
		  }
		  //**********************************************************************
