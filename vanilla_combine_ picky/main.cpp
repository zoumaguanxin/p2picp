...
//数据点关联，方案1
	...
	int counter_picky=0;
	int counter_max=60;
	while(convergence_counter<=2&&counter_picky<counter_max){
	  begin1=clock();
	    Eigen::Vector3f v1;
	    Eigen::Vector3f v2;
...
L1:            j = t % nPoint_current;
		...
		//picky
		if(pointNKNSquaredDistance[0]<search_r_up){
			    if(i==0)
			    {
				initPoints[i]=basic_cloud_ptr2->points[j];
				point_Distance[i]=pointNKNSquaredDistance[0];
				matchedPoints[i]=local_cloud_ptr->points[pointIdxNKNSearch[0]];
				indice[i]=pointIdxNKNSearch[0];
			    }
			    else
			    {
				  ...
				  {
					  if(ii<i){
						    if((indice[ii]==pointIdxNKNSearch[0]))
						    {
							    if(point_Distance[ii]>pointNKNSquaredDistance[0])
								    {
								      initPoints[ii]=basic_cloud_ptr2->points[j];
								      point_Distance[ii]=pointNKNSquaredDistance[0];
								      i--;
								      break;
								    }
								    else{
					    ...
					    initPoints[ii]=basic_cloud_ptr2->points[j];
					    matchedPoints[ii]=local_cloud_ptr->points[pointIdxNKNSearch[0]];
					    point_Distance[ii]=pointNKNSquaredDistance[0];
					    indice[ii]=pointIdxNKNSearch[0];
					  }
				  }
			    }       
		         i++;
