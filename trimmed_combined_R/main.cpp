...
//数据点关联，方案1
	...
	while(convergence_counter<=2&& iterative_counter<counter_max){
		...
		// cout<<"distance:"<<order_pointNKNSquareDdistance[i].data<<"         index:"<<order_pointNKNSquareDdistance[i].index<<endl;
		  initPoints[i]=ordered_basic_points[order_pointNKNSquareDdistance[i].index];
		  matchedPoints[i]=ordered_local_points[order_pointNKNSquareDdistance[i].index];
		  kdtree1.nearestKSearch(matchedPoints[i],K,pointIdxNKNSearch1,pointNKNSquaredDistance1);
		  double d;
		  d=pow(abs(middle_cloud_ptr->points[pointIdxNKNSearch1[0]].x-middle_cloud_ptr->points[order_pointNKNSquareDdistance[i].index].x),2)
			+pow(abs(middle_cloud_ptr->points[pointIdxNKNSearch1[0]].y-middle_cloud_ptr->points[order_pointNKNSquareDdistance[i].index].y),2)
			+pow(abs(middle_cloud_ptr->points[pointIdxNKNSearch1[0]].z-middle_cloud_ptr->points[order_pointNKNSquareDdistance[i].index].z),2);
		  d=sqrt(d);
		  if(d<d_threshold)
		  {
		   filter_initPoints->points.push_back(initPoints[i]);
		   filter_matchedPoints->points.push_back(matchedPoints[i]);
		  }
		}
